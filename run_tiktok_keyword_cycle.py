...
FETCH_URL = "http://127.0.0.1:5000/fetch/tiktok/auto"
...
def ensure_server_running() -> None:
    try:
        ...
        log(f"Dashboard health check failed before restart: {exc}")

    log("Dashboard is down. Launching local server.")
    ...
    raise RuntimeError("Dashboard did not recover after restart attempts.")
...
def main() -> int:
    ...
    log(
        "Starting TikTok keyword cycle: "
        f"keywords={KEYWORDS}, save_target={SAVE_TARGET_COUNT}, save_mode={SAVE_MODE}"
    )
    ...
    if failures:
        log(f"TikTok keyword cycle finished with failures={failures}.")
        return 1

    log("TikTok keyword cycle finished successfully.")
    return 0
